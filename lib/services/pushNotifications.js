...
async function getOrCreateDeviceId() {
  try {
    ...
    // A random fallback cannot be recovered at logout, leaving a live token.
    throw new Error('Unable to persist notification installation ID', { cause: error });
  }
}
...
async function getExpoPushToken({ requestPermission = true } = {}) {
  ...
  const allowed = value => value.granted || value.status === 'granted' ||
    [Notifications.IosAuthorizationStatus.PROVISIONAL, Notifications.IosAuthorizationStatus.EPHEMERAL].includes(value.ios?.status);

  if (!allowed(permission) && requestPermission && permission.canAskAgain !== false) {
...
async function registerDevice(options, generation) {
  try {
    const pushToken = await getExpoPushToken(options);
...
export function registerAdminPushToken(options) {
  if (registrationPending) return registrationPending;
  registrationPending = registerDevice(options, registrationGeneration)
    .finally(() => { registrationPending = null; });
...
export async function unregisterAdminPushToken() {
  ...
  if (registrationPending) await registrationPending.catch(() => {});
  try {
    const deviceId = await getOrCreateDeviceId();
